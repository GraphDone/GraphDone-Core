        ...
        createEdges: {
          ...
          edges: [{
            ...
            target: {
              ...
              title: workItems.find(n => n.id === input.target.connect.where.node.id)?.title || '',
              type: workItems.find(n => n.id === input.target.connect.where.node.id)?.type || 'TASK'
            },
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-5xl p-0 my-8 overflow-hidden text-left align-middle transition-all transform bg-gradient-to-br from-gray-800/98 via-gray-850/98 to-gray-900/98 backdrop-blur-2xl shadow-2xl rounded-2xl border border-gray-600/30 focus-within:ring-2 focus-within:ring-blue-500/50 animate-in slide-in-from-bottom-4 duration-300" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex">
            ...
            <div className="w-1/2 bg-gradient-to-b from-gray-800/50 to-gray-900/50 border-r border-gray-600/30 p-6">
              ...
              <div className="grid grid-cols-4 gap-4">
                {RELATIONSHIP_OPTIONS.filter(relation => relationshipFilter.includes(relation.type)).map((relation, index) => {
                  ...
                  return (
                    <div key={relation.type} className="relative group/tooltip">
                      <button
                        ...
                        disabled={isDisabled}
                        className={`w-full h-20 p-2.5 rounded-xl transition-all duration-300 border-2 backdrop-blur-sm relative overflow-hidden ${
                          isDisabled
                        ...
                        style={{
                          ...
                          ...(isSelected && !isDisabled && { boxShadow: `0 10px 25px -5px ${shadowClass}40, 0 8px 10px -6px ${shadowClass}30` })
                        }}
                      >
                        <div className="flex flex-col items-center justify-center space-y-1.5">
                          ...
                          <span className={`font-semibold text-[10px] text-center leading-tight transition-colors duration-300 ${
                            isDisabled ? 'text-gray-500' : isSelected ? relation.color : 'text-gray-300 group-hover:text-white'
                          }`}>
                            {relation.label}
                          </span>
                        </div>
                        ...
                        {isDisabled && (
                          <div className="absolute top-1 right-1 w-4 h-4 bg-red-500 text-white rounded-full flex items-center justify-center text-[8px] shadow-lg">
                            ✕
                          </div>
                        )}
                      </button>
                      ...
                      <div className={`absolute bottom-full ${tooltipPositionClass} mb-2 px-3 py-2 bg-gradient-to-br from-gray-900/98 to-black/98 backdrop-blur-xl rounded-lg border border-gray-600/50 shadow-2xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all duration-300 pointer-events-none z-50 whitespace-nowrap`}>
                        <div className="text-xs text-gray-300 leading-relaxed">
                          {isDisabled ? "Already exists" : relation.description}
                        </div>
            ...
            <div className="flex-1 flex flex-col bg-gradient-to-br from-gray-800/30 to-gray-900/40">
              {/* Scrollable Content */}
              ...
              <div className="flex justify-end space-x-3 p-6 border-t border-gray-700/30 bg-gray-800/40 backdrop-blur-sm">
                <button
                  onClick={onClose}
                  className="px-5 py-2.5 text-sm text-gray-300 bg-gray-700/50 rounded-xl hover:bg-red-600 hover:text-white transition-all duration-200 font-semibold hover:scale-105 border-2 border-gray-600/30 hover:border-red-500 flex items-center space-x-2 shadow-lg hover:shadow-red-500/20"
                >
                ...
                <button
                  ...
                  className={`px-5 py-2.5 text-sm rounded-xl transition-all duration-200 font-semibold flex items-center space-x-2 hover:scale-105 shadow-lg border-2 ${
                    selectedNodes.size === 0 || creatingConnection || isRelationshipDisabled(selectedRelationType) || (duplicateWarning && !duplicateWarning.isValid)
                      ? 'bg-gray-600/50 text-gray-400 cursor-not-allowed opacity-60 border-gray-500/30'
                      : duplicateWarning && duplicateWarning.isValid
                        ? 'bg-gradient-to-r from-yellow-600 via-orange-600 to-amber-600 text-white hover:from-yellow-500 hover:via-orange-500 hover:to-amber-500 border-yellow-400/50 hover:border-yellow-300 shadow-yellow-500/25 hover:shadow-xl hover:shadow-yellow-500/40'
                        : 'bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600 text-white hover:from-emerald-500 hover:via-green-500 hover:to-teal-500 border-emerald-400/50 hover:border-emerald-300 shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/40'
                  }`}
                  title={
                    duplicateWarning && !duplicateWarning.isValid
